...
test.describe("Flujo de registro de síntomas", () => {
  ...
  test("muestra la página de síntomas con el formulario de registro", async ({
    page,
  }) => {
    await page.goto(SYMPTOMS_URL);
    await page
      .getByText("Nuevo registro")
    ...
    await expect(
      page.getByRole("tab", { name: /registrar/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("tab", { name: /historial/i }),
    ).toBeVisible();
  });

  test("permite seleccionar un síntoma y ajustar intensidad", async ({
    page,
  }) => {
    await page.goto(SYMPTOMS_URL);
    await page
    ...
    const symptomBtn = page.getByRole("button", {
      name: /dolor abdominal/i,
    });
  ...
  test("permite seleccionar un síntoma y ajustar intensidad", async ({ page }) => {
    ...
    const slider = page.getByRole("slider", {
      name: /intensidad de dolor abdominal/i,
    });
  ...
  test("completa el formulario y guarda el registro", async ({ page }) => {
    ...
    await expect(
      page.getByText("Registro guardado correctamente"),
    ).toBeVisible({ timeout: 5000 });
  });

  test("muestra validación de temperatura fuera de rango", async ({
    page,
  }) => {
    await page.goto(SYMPTOMS_URL);
    await page
